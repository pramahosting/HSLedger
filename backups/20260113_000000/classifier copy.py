...
def classify_transactions(df: pd.DataFrame, show_progress=True) -> pd.DataFrame:
    ...
    # --- GL Account classification (use Ollama model if configured, otherwise keyword fallback) ---
    try:
        import os
        from backend.ai_model.classify_transaction import classify_gl_account

        gl_model = os.environ.get("GL_CLASSIFIER_MODEL")
        for idx in df.index:
            # description column is lowercase in this pipeline
            description = df.at[idx, "description"] if "description" in df.columns else ""
            # Only classify if GL Account is missing
            current_gl = df.at[idx, "GL Account"] if "GL Account" in df.columns else None
            if not current_gl or (isinstance(current_gl, float) and pd.isna(current_gl)):
                gl = classify_gl_account(description, model_name=gl_model)
                if gl:
                    df.at[idx, "GL Account"] = gl
    except Exception:
        # Safe: if classification fails, leave GL Account as-is
        pass
